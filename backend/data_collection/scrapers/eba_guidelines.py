...
URLS = [
    ("Payment services and electronic money", "https://www.eba.europa.eu/publications-and-media/publications?text=&document_type=250&media_topics=219&page=0"),
    ("Consumer protection", "https://www.eba.europa.eu/publications-and-media/publications?text=&document_type=250&media_topics=235&page=0"),
    ("AML and CFT", "https://www.eba.europa.eu/publications-and-media/publications?text=&document_type=250&media_topics=237&page=0"),
    ("Credit risk", "https://www.eba.europa.eu/publications-and-media/publications?text=&document_type=250&media_topics=234&page=0"),
    ("Digital Finance", "https://www.eba.europa.eu/publications-and-media/publications?text=&document_type=250&media_topics=5219&page=0"),
]
...
def ensure_text_column(cursor):
    """Add text column if the current eba_guidelines schema does not already have it."""
    cursor.execute("PRAGMA table_info(eba_guidelines)")
    columns = {row[1] for row in cursor.fetchall()}

    if "text" not in columns:
        cursor.execute("ALTER TABLE eba_guidelines ADD COLUMN text TEXT")
        ...
        for page in range(max_pages):
            ...
            for row in rows:
                ...
                if key not in seen:
                    ...
                    try:
                        cursor.execute("""
                            INSERT INTO eba_guidelines 
                            (id, category, title, published_date, document_url, url, text, type, source)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            pk_id,
                            row["category"],
